...
```{r sammy, results ='hide' ,echo =TRUE}
# concatenation `c()`
c(1, 3, 2, 5)
...
```{r lecture-plot, fig.show ='hold', fig.height=6}
# scatterplot
ggplot(data = eco, mapping = aes(x = usborn, y = income)) + 
  geom_point() +
  labs(title = 'My title', x = 'Percentage born in US', y = 'Annual income')
```
...
```{r}
# make me look pretty!
names_df <- data.frame(age_bracket = c('adult', 'adult', 'adolescent', 'infant'), age = c(32, 65, 13, 2))
names_df[,3] <- (c('jen', 'lin', 'georgia', 'niall'))
colnames(names_df)[3] <- "names"
names_df
```
