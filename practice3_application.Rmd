...
```{r, echo=TRUE, results='hide'}
std_beta_1_hat * (log(2))*100
```

The expected standard deviation of the change caused by doubling log(Area) is 0.776.
...
$$\text{Var}\varepsilon_i = (1 + \frac{x_i^2}{2})^2$$
...
# solution
# average estimate
avg_sigma_sq_hat <- sim_datasets %>% summarize(across(.cols = variance, .fns = mean))
```
...
```{r, echo = T}
# solution
new_var <- diag(1 + x_vals^2/2)^2
# compute variance covariance matrix
vcov_theoretical <- x_tx_inv %*% t(x_mx) %*% (new_var) %*% x_mx %*% x_tx_inv
